"""
build.py — converts TOP_BallByBall_Paper.md into a styled single-column HTML file
and renders it to PDF via headless Microsoft Edge. Re-run after any edit to the
markdown source; prints the final page count. Same tooling/style as paper/build.py.
...
MD_PATH = os.path.join(HERE, "TOP_BallByBall_Paper.md")
HTML_PATH = os.path.join(HERE, "TOP_BallByBall_Paper.html")
PDF_PATH = os.path.join(HERE, "TOP_BallByBall_Paper.pdf")
...
<head>
<meta charset="utf-8">
<title>Ball-by-Ball Tactical Decision Support in T20 Cricket</title>
<style>{css}</style>
</head>
<body>
{body}
