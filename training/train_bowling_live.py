"""
train_bowling_live.py
-----------------------
Retrains all 15 bowling models (W1-W15) for the "live" system: an isolated
sibling of train_bowling.py (paper 1, models/bowling/) and
train_bowling_ballbyball.py (paper 2, models/bowling_ballbyball/) - neither
of those is modified, run, or read from here.

Two things change relative to train_bowling.py:

1. Row selection: every model trains/evaluates on (near-)every delivery in
   the dataset instead of only the last ball of the four strategic-timeout
   overs, via a local bowling_full_snapshot() (identical in spirit to
   train_bowling_ballbyball.py's bowling_ballbyball_snapshot() - copied
   verbatim, along with that file's two proven fixes: the W3 NaN-drop before
   XGBoost, and W9/W15's phase-column filters replacing the over==15/over==5
   snapshot filters).

2. New player-identity features, split by algorithm family:
   - Group A (LightGBM / XGBoost(enable_categorical) / CatBoost - already
     natively handle high-cardinality categoricals): W1, W2, W3, W8, W9,
     W11, W14, W15 get "batter"/"bowler" added directly to their categorical
     feature list (EXTRA_CATEGORICAL/EXTRA_FEATURES below).
   - Group B (LogisticRegression / GaussianNB / RandomForestClassifier -
     one-hotting ~600+ bowler / ~800+ batter names is impractical): W4, W6,
     W7, W13 instead get 7 DERIVED NUMERIC columns looked up from
     decision_engine/config/player_profiles.json (built by
     build_player_lookups.py, train-matches only - leakage safe).
   - W5, W10 (fixed-weight formulas) and W12 (lookup) only get the snapshot
     change - no feature vector exists for these, so no player features.

Same algorithms, hyperparameters, target definitions, and match-level
(seed=42) split as train_bowling.py - only row selection and features
change. Outputs go to NEW, isolated paths:
  - models/bowling_live/
  - data/output_data_live/bowling/
"""
...
MODEL_ROOT = os.path.join(HERE, "..", "models", "bowling_live")
OUT_ROOT = os.path.join(HERE, "..", "data", "output_data_live", "bowling")
...
# Group A: raw categorical player-identity columns (local to this file only -
# the shared BOWLING_CATEGORICAL / BOWLING_SNAPSHOT_FEATURES in common.py are
# left untouched).
EXTRA_CATEGORICAL = BOWLING_CATEGORICAL + ["batter", "bowler"]
EXTRA_FEATURES = BOWLING_SNAPSHOT_FEATURES + ["batter", "bowler"]

# Group B: derived numeric player-profile columns.
PLAYER_NUMERIC_COLS = [
    "bowler_career_economy", "bowler_career_dot_pct", "bowler_career_boundary_pct",
    "bowler_career_wickets_per_over", "batter_career_strike_rate",
    "batter_career_dismissal_rate", "batter_career_boundary_pct",
]
...
def bowling_full_snapshot(df, require_future_balls=False):
    ...
    ball of the four strategic-timeout overs. Copied verbatim from
    train_bowling_ballbyball.py's bowling_ballbyball_snapshot()."""
    snap = df.copy()
...
def _prep_snapshot(df, require_future_balls=True):
    ...
    return bowling_full_snapshot(df, require_future_balls=require_future_balls)


def add_bowler_profile_features(df):
    """Group B: 4 derived numeric columns looked up from PLAYER_PROFILES via
    the row's own 'bowler' column, with a global fallback for unseen bowlers."""
    df = df.copy()
    bowlers = PLAYER_PROFILES["bowlers"]
    fallback = PLAYER_PROFILES["bowler_fallback"]
    df["bowler_career_economy"] = df["bowler"].map(lambda b: bowlers.get(b, fallback)["career_economy"])
    df["bowler_career_dot_pct"] = df["bowler"].map(lambda b: bowlers.get(b, fallback)["dot_ball_percentage"])
    df["bowler_career_boundary_pct"] = df["bowler"].map(lambda b: bowlers.get(b, fallback)["boundary_percentage"])
    df["bowler_career_wickets_per_over"] = df["bowler"].map(lambda b: bowlers.get(b, fallback)["wickets_per_over"])
    return df


def add_batter_profile_features(df):
    """Group B: 3 derived numeric columns looked up from PLAYER_PROFILES via
    the row's own 'batter' column, with a global fallback for unseen batters."""
    df = df.copy()
    batters = PLAYER_PROFILES["batters"]
    fallback = PLAYER_PROFILES["batter_fallback"]
    df["batter_career_strike_rate"] = df["batter"].map(lambda b: batters.get(b, fallback)["strike_rate"])
    df["batter_career_dismissal_rate"] = df["batter"].map(lambda b: batters.get(b, fallback)["dismissal_rate"])
    df["batter_career_boundary_pct"] = df["batter"].map(lambda b: batters.get(b, fallback)["boundary_pct"])
    return df


def _native_cat_train_test(snap, target_col, extra_filter=None):
    """Shared plumbing for every Group-A 'clean template' W-model: snapshot ->
    match split -> native-categorical X/y for train and test, using
    EXTRA_CATEGORICAL/EXTRA_FEATURES (batter+bowler included)."""
    if extra_filter is not None:
        snap = snap[extra_filter(snap)].copy()
    train_mask, test_mask = match_level_split(snap)
    vocab = fit_native_categories(snap.loc[train_mask], EXTRA_CATEGORICAL)
    X_train = apply_native_categories(snap.loc[train_mask, EXTRA_FEATURES], EXTRA_CATEGORICAL, vocab)
    X_test = apply_native_categories(snap.loc[test_mask, EXTRA_FEATURES], EXTRA_CATEGORICAL, vocab)
...
# ---------------------------------------------------------------------------
# W1 : Economy Predictor (LightGBM Regressor) - Group A
# ---------------------------------------------------------------------------
def train_w1(df):
    ...
    snap = bowling_full_snapshot(full, require_future_balls=False)
    ...
    model.fit(X_train, y_train, categorical_feature=EXTRA_CATEGORICAL)
    ...
    save_feature_importance(EXTRA_FEATURES, model.feature_importances_,
...
# ---------------------------------------------------------------------------
# W2 : Wicket Probability Predictor (CatBoost Classifier) - Group A
# ---------------------------------------------------------------------------
def train_w2(df):
    ...
    train_mask, test_mask = match_level_split(snap)
    cat_idx = [EXTRA_FEATURES.index(c) for c in EXTRA_CATEGORICAL]
    X = snap[EXTRA_FEATURES].copy()
    ...
    joblib.dump(model, os.path.join(mod_dir, f"{mid}_model.pkl"))
    save_json({"categorical_features": EXTRA_CATEGORICAL}, os.path.join(mod_dir, f"{mid}_categories.json"))
    save_metrics(metrics, os.path.join(out_dir, f"{mid}_model_metrics.csv"))
...
# ---------------------------------------------------------------------------
# W3 : Dot Ball Pressure (XGBoost Regressor) - Group A
# ---------------------------------------------------------------------------
def train_w3(df):
...
# ---------------------------------------------------------------------------
# W4 : Variation Control (Logistic Regression, one-hot) - Group B
# ---------------------------------------------------------------------------
def train_w4(df):
    ...
    snap = add_bowler_profile_features(snap)
    snap = add_batter_profile_features(snap)

    train_mask, test_mask = match_level_split(snap)
    numeric = [c for c in BOWLING_SNAPSHOT_FEATURES if c not in BOWLING_CATEGORICAL] + PLAYER_NUMERIC_COLS
...
# ---------------------------------------------------------------------------
# W6 : Line & Length Consistency (Gaussian Naive Bayes, one-hot) - Group B
# ---------------------------------------------------------------------------
def train_w6(df):
...
# ---------------------------------------------------------------------------
# W7 : Spin Control (Random Forest, one-hot) - Group B
# ---------------------------------------------------------------------------
def train_w7(df):
...
# ---------------------------------------------------------------------------
# W8 : Yorker Effectiveness (CatBoost Classifier) - Group A
# ---------------------------------------------------------------------------
def train_w8(df):
...
# ---------------------------------------------------------------------------
# W9 : Death Over Accuracy (LightGBM Classifier) - Group A, phase-restricted
# ---------------------------------------------------------------------------
def train_w9(df):
    ...
    print("  the over=15 timeout snapshot) - phase-scoping kept, snapshot restriction")
    print("  dropped, same fix as train_bowling_ballbyball.py's W9.")
...
# ---------------------------------------------------------------------------
# W11 : Run-Containment (LightGBM Regressor) - Group A
# ---------------------------------------------------------------------------
def train_w11(df):
...
# No feature vector, no player-identity features - snapshot switch only.
# ---------------------------------------------------------------------------
def train_w12(df):
    ...
    full = add_future_spell_stats(full)
    test_snap = bowling_full_snapshot(full, require_future_balls=True)
...
# ---------------------------------------------------------------------------
# W13 : L/R Matchup Bias (Logistic Regression, one-hot) - Group B
# ---------------------------------------------------------------------------
def train_w13(df):
...
# ---------------------------------------------------------------------------
# W14 : Economy Trend Analysis (LightGBM Regressor) - Group A
# ---------------------------------------------------------------------------
def train_w14(df):
...
# ---------------------------------------------------------------------------
# W15 : Powerplay Containment (LightGBM Classifier) - Group A, phase-restricted
# ---------------------------------------------------------------------------
def train_w15(df):
    ...
    print("  over=5 timeout snapshot) - same fix as train_bowling_ballbyball.py's W15.")
...
def main():
    ...
    save_json(RESULTS, os.path.join(OUT_ROOT, "..", "bowling_live_training_summary.json"))
    print("All bowling_live models trained. Summary written to bowling_live_training_summary.json")
